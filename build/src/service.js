...
class Service {
    ...
    generateGenericInbound(config, channel, inboundsDomain, deployUuid) {
        ...
        [vhostPath, inboundPath, linkPath].forEach((p) => {
            utils_1.createPath(p);
        });
        let params = {
            "domain": config.domain,
            "name": channel.name,
            "deployUuid": deployUuid,
            "inboundUuid": uuid_1.v4().replace("_", "-")
        };
        return utils_1.createElementFromTemplate(vhostTemplatePath, vhostPath, params)
            .then(() => {
            return utils_1.createElementFromTemplate(inboundTemplatePath, inboundPath, params);
        })
            .then(() => {
            return utils_1.createElementFromTemplate(linkTemplatePath, linkPath, params);
        });
    }
}
